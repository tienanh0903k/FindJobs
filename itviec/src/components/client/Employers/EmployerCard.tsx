...
export const EmployerCard: React.FC<IEmployerCard> = ({ job }: IEmployerCard) => {
	...
	return (
		<div className="
			...
			text-center mt-4 bg-transparent border border-gray-200 p-4
			hover:border-blue-500 cursor-pointer hover:shadow-lg"
			onClick={
				() => {
					router.push(`/company/${job._id}`);
				}
			}
		...
		>
			...
			<div className="mt-6 flex-grow text-black z-20">
				<h3 className="text-black text-lg font-semibold">{job?.name}</h3>
			...
			<div className="w-full rounded-b-lg z-20 border-t-1 border-gray-300 flex justify-between items-center mt-4">
				...
				<div className="flex items-center text-it-black">
					<GoDotFill className="text-green-500" />1 Việc làm
					<svg className="w-5 h-5 ml-1 text-gray-700"></svg>
				</div>
			</div>
		</div>
	);
