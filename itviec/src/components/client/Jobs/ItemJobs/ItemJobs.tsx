...
const ItemJobs = ({ job }: ItemJobsProps) => {
	...
	const { position, salary, location, companyId } = job || {};
	...
	return (
		<div className="w-90 max-w-sm rounded-lg border border-gray-300 shadow-md p-4 m-4 overflow-hidden bg-white transition-all hover:shadow-xl hover:border-blue-500 hover:bg-gray-50 hover:cursor-pointer">
			<div className="flex items-center mb-4">
				...
				<Image src={job?.companyId?.logo || 'https://via.placeholder.com/150'} alt="Logo cong ty" width={50} height={50} />
				<div className="ml-4">
					...
					<p
						className="text-gray-600 text-sm line-clamp-1 whitespace-nowrap"
						title={job?.companyId?.name}
					>
						{truncateString(job?.companyId?.name, 36)} 
					</p>
				</div>
			</div>
...
// 				{/* <span className="text-white bg-orange-500 px-2 py-1 text-sm rounded">HOT</span> */}
// 			</h3>
// 			<p className="text-gray-600 line-clamp-1">{job?.companyId?.name}</p>
// 		</div>
// 	</div>
// 	<div className="flex justify-between items-center">
// 		<p className="text-gray-600">{job.salary}</p>
